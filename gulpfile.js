...
gulp.task('clean', function() {
    return del(['public', 'assets/css/fse.css']);
});
...
gulp.task('vendor:build', function() {
    ...
    const fontStream = gulp.src(['./assets/fonts/font-awesome/**/*.*']).pipe(gulp.dest('./public/assets/fonts/font-awesome'));
...
gulp.task('css:minify', gulp.series('scss', function cssMinify() {
    return gulp.src("./assets/css/fse.css")
        .pipe(cleanCSS())
        .pipe(rename({
            suffix: '.min'
        }))
...
// Replace HTML block for Js and Css file upon build and copy to /public
gulp.task('replaceHtmlBlock', function () {
    return gulp.src(['*.html'])
        .pipe(htmlreplace({
            'js': 'assets/js/fse.min.js',
            'css': 'assets/css/fse.min.css'
        }))
...
gulp.task('dev', function browserDev(done) {
    ...
    gulp.watch('assets/js/fse.js', gulp.series('js:minify', function jsBrowserReload (done) {
        browserSync.reload();
        done();
    }));
